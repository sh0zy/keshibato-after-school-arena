...
class AudioManager {
 ...
 private buffer(id:string){if(!this.buffers.has(id))this.buffers.set(id,fetch(assetUrl(`/audio/${id}.wav`)).then(r=>{if(!r.ok)throw new Error(`音声 ${id} を読み込めませんでした`);return r.arrayBuffer()}).then(b=>this.context!.decodeAudioData(b)).catch(e=>{this.buffers.delete(id);throw e}));return this.buffers.get(id)!}
